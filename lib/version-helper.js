...
function printVersions(log = console.log) {
  ...
  log('@rayyone/cli version: %s', ver);
  log('\n@rayyone/* dependencies:');
  for (const d in templateDeps) {
    if (d.startsWith('@rayyone/') && d !== '@rayyone/cli') {
...
async function checkDependencies(generator) {
  ...
  const dependentPackage = '@rayyone/core';
  ...
  const isLBProj = isUpdate
    ? projectDepsNames.some(n => n.startsWith('@rayyone/'))
    : projectDepsNames.includes(dependentPackage);

  if (!isLBProj) {
    const err = new Error(
      'No `@rayyone/core` package found in the "dependencies" section of ' +
  ...
  if (!found) {
    ...
    if (generator.command === 'update') {
      generator.log(chalk.green(`The project dependencies are compatible with @rayyone/cli@${cliPkg.version}`));
    }
    return;
  }

  const originalCliVersion = generator.config.get('update') || '<unknown>';
  generator.log(chalk.red(g.f('The project was originally generated by @rayyone/cli@%s.', originalCliVersion)));

  generator.log(chalk.red(g.f('The following dependencies are incompatible with @rayyone/cli@%s:', cliPkg.version)));
  for (const s in incompatibleDeps) {
    generator.log(s);
    for (const d in incompatibleDeps[s]) {
      generator.log(chalk.yellow('- %s: %s (cli %s)'), d, ...incompatibleDeps[s][d]);
    }
  }
...
function updateDependencies(generator) {
  const pkg = generator.packageJson || generator.fs.readJSON(generator.destinationPath('package.json'));
  const depUpdates = [];
  for (const d in templateDeps) {
    if (pkg.dependencies && pkg.dependencies[d] && pkg.dependencies[d] !== templateDeps[d]) {
      depUpdates.push(`- Dependency ${d}: ${pkg.dependencies[d]} => ${templateDeps[d]}`);
      pkg.dependencies[d] = templateDeps[d];
    }
    if (pkg.devDependencies && pkg.devDependencies[d] && pkg.devDependencies[d] !== templateDeps[d]) {
      depUpdates.push(`- DevDependency ${d}: ${pkg.devDependencies[d]} => ${templateDeps[d]}`);
      pkg.devDependencies[d] = templateDeps[d];
    }
    if (pkg.peerDependencies && pkg.peerDependencies[d] && pkg.peerDependencies[d] !== templateDeps[d]) {
      depUpdates.push(`- PeerDependency ${d}: ${pkg.devDependencies[d]} => ${templateDeps[d]}`);
      pkg.devDependencies[d] = templateDeps[d];
    }
  }
  ...
  generator.log(chalk.red('Upgrading dependencies may break the current project.'));
...
async function checkCliVersion(log = console.log) {
  const latestCliVersion = await latestVersion('@rayyone/cli');
